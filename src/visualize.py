# Adapted and amended from nolearn/Lasagne
...
def plot_conv_weights(layer, filename="", figsize=(6, 6)):
    ...
    for feature_map in range(shape[1]):
        ...
        if len(filename)>0:
            plt.savefig(filename+str(feature_map)+'.png') # !
...
def plot_conv1_weights(layer, filename="", figsize=(6, 6)):
    ...
    for i, (r, c) in enumerate(product(range(nrows), range(ncols))):
        if i >= shape[0]:
            break
        axes[r, c].imshow(W[i,:], interpolation='nearest')
        
    if len(filename)>0:
        plt.savefig(filename) # !
        
def plot_conv_activity(layer, x, filename="", figsize=(6, 8)):
